...
export async function createConsumption(environment: Environment) {
  ...
  const res = await api.post(`/environment-record`, formData, {
    headers: {
...
export async function getConsumptionList() {
  const res = await api.get("/environment-record");
  return res.data;
}

export async function getConsumptionAssignedList() {
  const res = await api.get("/environment-record-assign-task");
  return res.data;
}
...
export async function updateConsumption(environment: Environment) {
  ...
  const res = await api.post(
    `/environment-record/${environment.id}/update`,
    formData,
    {
      headers: {
        "Content-Type": "multipart/form-data",
...
export async function deleteConsumption(id: string) {
  const res = await api.delete(`/environment-record/${id}/delete`);
  return res.data;
}

export async function fetchConsumptionCategories() {
  const res = await api.get(`/consumption-get-categories`);
  return res.data;
}

export async function fetchConsumptionUnit(category: string) {
  const res = await api.get(`/consumption-get/${category}/units`);
  return res.data;
}

export async function fetchConsumptionSource(category: string) {
  const res = await api.get(`/consumption-get/${category}/sources`);
  return res.data;
}

export async function fetchConsumptionAssignee() {
  const res = await api.get(`/environment-record-assignee`);
  return res.data;
}
