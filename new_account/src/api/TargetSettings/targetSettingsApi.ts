...
export const createTargetSettings = async (targetSettings: TargetSettings) => {
  ...
  const res = await api.post("/target-setting", formData, {
    headers: {
...
export const updateTargetSettings = async (targetSettings: TargetSettings) => {
  ...
  try {
    const response = await api.post(
      `/target-setting/${targetSettings.id}/update`,
      formData,
...
export const deleteTargetSettings = async (id: string) => {
  const res = await api.delete(`/target-setting/${id}/delete`);
  return res.data;
};

export async function getTargetSettings() {
  const res = await api.get("/target-setting");
  return res.data;
}

export async function getAssignedTargetSettings() {
  const res = await api.get("/target-setting-assign-task");
  return res.data;
}

export async function getApproverAndAssignee() {
  const res = await api.get("/target-setting-assignee");
  return res.data;
}

export async function getSource() {
  const res = await api.get("/ts-sources");
  return res.data;
}

export async function fetchMainTsCategory() {
  const res = await api.get("/ts-categories");
  return res.data;
}

export async function fetchPossibilityCategory(categoryName: string) {
  const res = await api.get(
    `/categories/${categoryName}/possibilityCategory`
  );
  return res.data;
}
...
export async function fetchOpportunity(subCategoryName: string) {
  const res = await api.get(
    `/subcategories/${subCategoryName}/opportunities`
  );
  return res.data;
}
