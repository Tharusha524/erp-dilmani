...
export async function getMaternityRegistersList() {
  const res = await api.get("/benefit-request");
  return res.data;
}
...
export const createMaternityRegister = async (
  ...
  const res = await api.post("/benefit-request", formData, {
    headers: {
...
export const updateMaternityRegister = async (
  ...
  const res = await api.post(
    `/benefit-request/${maternityRegister.id}/update`,
    formData,
    {
      headers: {
        "Content-Type": "multipart/form-data",
...
export const deleteMaternityRegister = async (id: string) => {
  const res = await api.delete(`/benefit-request/${id}/delete`);
  return res.data;
};
