...
export const createSustainabilityReport = async (
  ...
  try {
    const res = await api.post("/sdg-report", formData, {
      headers: {
...
export const updateSustainabilityReport = async (
  ...
  try {
    const res = await api.post(
      `/sdg-report/${sustainability.id}/update`,
      formData,
      {
        headers: {
          "Content-Type": "multipart/form-data",
...
export const deleteSustainabilityRecord = async (id: string) => {
  const res = await api.delete(`/sdg-report/${id}/delete`);
  return res.data;
};

export async function getSustainabilityList() {
  const res = await api.get("/sdg-report");
  return res.data;
}

export async function getAdditionalSDGList() {
  const res = await api.get("/additional-SDG");
  return res.data;
}

export async function getAlignmentSDGList() {
  const res = await api.get("/alignment-SDG");
  return res.data;
}
...
export async function getMaterialitytypeList() {
  const res = await api.get("/materiality-type");
  return res.data;
}
...
export async function getSdgValueList() {
  const res = await api.get("/sdg-value");
  return res.data;
}

export async function getImpactList() {
  const res = await api.get("/impact-type");
  return res.data;
}

export async function fetchImpactUnit(impactType: string) {
  const res = await api.get(`/impact-types/${impactType}/impactUnit`);
  return res.data;
}
