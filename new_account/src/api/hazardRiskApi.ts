...
export async function getHazardRiskList() {
  const res = await api.get("/hazard-and-risk");
  return res.data;
}

export async function getAssignedHazardRiskList() {
  const res = await api.get("/hazard-risks-assign-task");
  return res.data;
}
...
export const createHazardRisk = async (hazardRisk: HazardAndRisk) => {
  ...
  const res = await api.post("/hazard-and-risk", formData, {
    headers: {
...
export const updateHazardRisk = async (hazardRisk: HazardAndRisk) => {
  ...
  try {
    const response = await api.post(
      `/hazard-risk/${hazardRisk.id}/update`,
      formData,
...
export const deleteHazardRisk = async (id: string) => {
  const res = await api.delete(`/hazard-risk/${id}/delete`);
  return res.data;
};
