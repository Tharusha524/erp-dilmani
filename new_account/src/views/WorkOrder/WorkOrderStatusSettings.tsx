...
export default function WorkOrderStatusSettings() {
  ...
  const assignedUsersByStatus = useMemo(() => {
    const map: Record<number, { user_id: number; user_name: string }[]> = {};
    assignments.forEach((a) => {
      if (!map[a.status_id]) map[a.status_id] = [];
      map[a.status_id].push({ user_id: a.user_id, user_name: a.user_name });
    });
    return map;
  }, [assignments]);
  ...
  const { mutate: addAssignedUser } = useMutation({
    mutationFn: ({ statusId, userId }: { statusId: number; userId: number }) =>
      assignWorkOrderStatusUser(statusId, userId),
    onSuccess: () => {
  ...
  const { mutate: removeAssignedUser } = useMutation({
    mutationFn: ({ statusId, userId }: { statusId: number; userId: number }) =>
      unassignWorkOrderStatusUser(statusId, userId),
    onSuccess: () => {
      invalidateAssignments();
    ...
    <FormPageLayout>
      <Box sx={{ p: { xs: 2, md: 3 } }}>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          Manage the day-by-day status workflow for Factory work orders, and assign the people authorized to
          act on each status (leave unassigned to keep it open to everyone). Printing and Embroidery don't use
          a status workflow yet — support for those can be added here later.
        </Typography>
                    ...
                    <Table size="small">
                      <TableHead>
                        <TableRow>
                          ...
                          <TableCell sx={{ fontWeight: "bold", width: 260 }}>Assigned People</TableCell>
                        ...
                        {groupStatuses.map((s) => {
                          ...
                          return (
                            <TableRow key={s.id}>
                              ...
                              <TableCell>
                                <AssignedUsersEditor
                                  assigned={assignedUsersByStatus[s.id] || []}
                                  users={users}
                                  onAdd={(userId) => addAssignedUser({ statusId: s.id, userId })}
                                  onRemove={(userId) => removeAssignedUser({ statusId: s.id, userId })}
                                />
                              </TableCell>
