...
export default function OutstandingCostCenters() {
  ...
  const runSearch = async (overrides?: { onlyOverdue?: boolean }) => {
    ...
    try {
      ...
      console.error("Outstanding costCenters search failed:", error);
      setRows([]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <Stack spacing={2}>
      ...
      >
        <Box>
          <PageTitle title="Outstanding CostCenters" />
          <Breadcrumb
            breadcrumbs={[
              { title: "Home", href: "/dashboard" },
              { title: "CostCenters", href: "/costCenter/transactions" },
              { title: "Outstanding CostCenters" },
            ]}
          />
        </Box>
