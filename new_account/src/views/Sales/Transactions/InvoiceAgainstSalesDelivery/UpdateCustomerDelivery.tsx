...
export default function UpdateCustomerDelivery() {
  ...
  const handleUpdate = async () => {
    ...
    const invalidRow = rows.find(
      (r) => r.deliveryQty > Math.max((r.ordered ?? 0) - (r.invoice ?? 0), 0)
    );
    if (invalidRow) {
      alert(
        `Delivery quantity for item ${invalidRow.itemCode} exceeds remaining quantity.`
      );
      return;
    }
    ...
    try {
      ...
      await updateDebtorTran(recordId, transData);

      const orderData = allSalesOrders.find(
        (o: any) => String(o.order_no) === String(salesOrder.order_no)
      );
      if (orderData) {
        await updateSalesOrder(orderData.order_no, {
          ...orderData,
          version: (orderData.version || 0) + 1,
        });
      }

      for (const row of rows) {
        if (!row.detailId) continue;
        await updateDebtorTransDetail(row.detailId, {
      ...
      navigate("/sales/transactions/update-customer-delivery/success", {
        state: { deliveryNo, reference, date },
      });
    } catch (error) {
