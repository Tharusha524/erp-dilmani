    ...
    const loadCompanySetup = async () => {
      try {
        ...
        if (data && data.length > 0) {
          ...
          setCompanyId(String((company as Record<string, unknown>).id ?? ""));
          setLogoPreviewUrl(
            ((company as Record<string, unknown>).company_logo_url as string | undefined) ??
            null
          );
        }
      } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    if (isValid) {
      try {
        ...
        console.error(error);
        // API interceptor shows a user-friendly MessageDialog
      }
    }
  };
          ...
          <Grid item xs={12} md={6}>
            <Stack spacing={2}>
              ...
              <Box sx={{ mt: 2 }}>
                ...
                <input type="file" name="new_company_logo" accept="image/*" onChange={handleChange} />
                {logoPreviewUrl && (
                  <Box sx={{ mt: 1 }}>
                    <img src={logoPreviewUrl} alt="Company logo preview" style={{ maxHeight: 80, maxWidth: 200 }} />
                  </Box>
                )}
              </Box>
