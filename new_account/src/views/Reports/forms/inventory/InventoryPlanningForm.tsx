...
interface InventoryPlanningFormData {
  itemCategory: string;
  location: string;
  comments: string;
  orientation: string;
...
export default function InventoryPlanningForm() {
  const muiTheme = useTheme();
  const isMobile = useMediaQuery(muiTheme.breakpoints.down("sm"));

  const [formData, setFormData] = useState<InventoryPlanningFormData>({
  ...
  const [errors, setErrors] = useState<Partial<InventoryPlanningFormData>>({});
  ...
  // Simple validation
  const validate = () => {
    let newErrors: Partial<InventoryPlanningFormData> = {};
