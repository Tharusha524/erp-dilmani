...
class BudgetController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        $rows = BudgetTrans::query()
            ...
            ->where('cost_center_id', $request->integer('cost_center_id', 0))
            ->where('cost_center2_id', $request->integer('cost_center2_id', 0))
            ->orderBy('tran_date')
            ->get();
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $costCenterId = (int) ($validated['cost_center_id'] ?? 0);
        $costCenter2Id = (int) ($validated['cost_center2_id'] ?? 0);
        $saved = [];
