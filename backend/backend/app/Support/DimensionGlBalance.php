...
class DimensionGlBalance
{
    public static function sum(int $dimensionId, ?string $fromDate = null, ?string $toDate = null): float
    {
        ...
        self::applyDimensionFilter($query, $dimensionId);
     ...
     */
    public static function byAccount(int $dimensionId, ?string $fromDate = null, ?string $toDate = null): Collection
    {
        if (! Schema::hasTable('gl_trans')) {
            return collect();
        }
    ...
    public static function hasTransactions(int $dimensionId): bool
    {
        if (! Schema::hasTable('gl_trans')) {
            return false;
        }
    ...
    private static function applyDimensionFilter(Builder $query, int $dimensionId): void
    {
        $query->where(function (Builder $q) use ($dimensionId) {
            $applied = false;

            if (Schema::hasColumn('gl_trans', 'dimension_id')) {
                $q->where('gl_trans.dimension_id', $dimensionId);
                $applied = true;
            } elseif (Schema::hasColumn('gl_trans', 'dimension')) {
                $q->where('gl_trans.dimension', $dimensionId);
                $applied = true;
            }

            if (Schema::hasColumn('gl_trans', 'dimension2_id')) {
                if ($applied) {
                    $q->orWhere('gl_trans.dimension2_id', $dimensionId);
                } else {
                    $q->where('gl_trans.dimension2_id', $dimensionId);
                }
            }
        });
    }
